...
class SalesSubStageController {
  static handleSubStageChange = async (
    newSalesSubStageId,
    opportunityId,
    updateDate,
    session
    ...
    if (
      !allSalesSubStages.find(
        (subStage) => subStage._id.toString() == newSalesSubStageId
      )
    )
      throw new ClientError(
        "handleSubStageChange",
        errors.subStage.INVALID_STAGE
      );
    const opportunity = await OpportunityMasterModel.findById(opportunityId)
      .populate("stageHistory stageHistory.stage")
      .session(session);
    ...
    const subStage = allSalesSubStages.find(
      (subStage) => subStage._id == newSalesSubStageId
    );
    if (subStage.salesStage.toString() != opportunity.salesStage.toString())
      throw new ClientError(
        "handleSubStageChange",
        errors.subStage.MISMATCHED_STAGE
      );

    //Creating new subStageHistory
    const newSubStageHistory = new SubStageHistoryModel({
      subStage: newSalesSubStageId,
