...
import { ServerError, ClientError } from "../../utils/customErrorHandler.utils.js";
import RoleModel from "../../models/RoleModel.js";
import PermissionModel from "../../models/PermissionModel.js";
import { createPermissionEntity } from "../../utils/createEntity.js";
...
class RoleController {
    ...
    static getAllPermissions = catchAsyncError(async (req, res, next) => {
        const permissions = await PermissionModel.find();

        if (!permissions || permissions.length === 0) {
            throw new ServerError("NotFound", "Permissions not found");
        }
    ...
    static getRole = catchAsyncError(async (req, res, next) => {
        ...
        if (!mongoose.Types.ObjectId.isValid(id)) {
            return next(new ClientError("InvalidId", "The provided role ID is invalid."));
        }
    ...
    static createRole = catchAsyncError(async (req, res, next) => {
        ...
        const existingEntity = await PermissionModel.findOne({ entity: roleName });
        ...
        await createPermissionEntity(roleName);
    ...
    static editRolePermissions = catchAsyncError(async (req, res, next) => {
        try {
            ...
            for (const update of permissionUpdates) {
                const permission = role.permissions.find(perm => perm.entity.toString() === update.entity);

                if (!mongoose.Types.ObjectId.isValid(update.entity)) {
                    return next(new ClientError("InvalidId", "The provided permission entity ID is invalid."));
                }
                const actualPermission = await PermissionModel.findById(update.entity);

                if (!actualPermission) {
                    throw new ClientError("Not Found", "The specified permission entity does not exist.");
                }

                const actualActions = actualPermission.actions; // Get the actual actions for the entity

                // Check if allowedActions are valid
                const invalidActions = update.allowedActions.filter(action => !actualActions.includes(action));
                if (invalidActions.length > 0) {
                    throw new ClientError("Invalid Action", `The following actions are not valid: ${invalidActions.join(', ')}`);
                }
